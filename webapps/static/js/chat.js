...
async function ask_gpt2(message){
  message_input.value = ``;
  ...
  try {
    ...
    if(response['status']) {
     ...
     if (error.name !== `AbortError`) {
       document.getElementById(`gpt_${window.token}`).innerHTML = `oops ! something went wrong, please try again / reload. [stacktrace in console]`;
     } else {
